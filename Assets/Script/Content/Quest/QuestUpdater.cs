...
{
    Queue<Quest> _completedCacheQuests;
    WaitForSeconds _interval;
    Coroutine _updateCoroutine;

    private void Start()
    {
        _completedCacheQuests = new Queue<Quest>();
        _interval = new WaitForSeconds(1f);

        Managers.Quest.onQuestCompleted += AddCompletedQuest;

        _updateCoroutine = StartCoroutine(UpdateQuestsRoutine());
    ...
    {
        _completedCacheQuests.Enqueue(quest);
    }

    IEnumerator UpdateQuestsRoutine()
    {
        while(true)
        {
            UpdateQuestToServer();
    ...
    // 완료된 퀘스트를 서버에 업데이트 한다.
    public void UpdateQuestToServer()
    {
        if(Managers.Network.IS_ENABLE_NETWORK == false)
            _completedCacheQuests.Clear();
        

        if (_completedCacheQuests.Count <= 0)
            return;
        ...
        foreach(Quest quest in _completedCacheQuests)
        {
