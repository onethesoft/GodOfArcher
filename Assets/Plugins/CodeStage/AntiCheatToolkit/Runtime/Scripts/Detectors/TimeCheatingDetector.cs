...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class TimeCheatingDetector : ACTkDetectorBase<TimeCheatingDetector>
	{
		...
		public struct OnlineTimeResult
		{
			...
			{
				if (success)
					return $"{nameof(onlineSecondsUtc)}: {onlineSecondsUtc}";
		...
		private static Lazy<NonValidatingCertificateHandler> CachedCertificateHandler { get; } =
			new Lazy<NonValidatingCertificateHandler>(InstantiateCertificateHandler);
		...
		/// </summary>
		public OnlineTimeResult LastOnlineTimeResult { get; private set; }
		...
		{
			return new NonValidatingCertificateHandler();
		}
		...
		private static UnityWebRequest GetWebRequest(Uri uri, RequestMethod method)
		{
			var request = new UnityWebRequest(uri, method == RequestMethod.Head ? UnityWebRequest.kHttpVerbHEAD : UnityWebRequest.kHttpVerbGET)
			{
				...
				certificateHandler = CachedCertificateHandler.Value,
		...
		private static void FillRequestResult(UnityWebRequest request, ref OnlineTimeResult result)
		{
			...
			{
				result.SetError("UnityWebRequest Error: " + request.error, request.responseCode);
			}
			else
			{
		...
		/// </summary>
		/// <returns>True if you can call the ForceCheck and False otherwise.</returns>
		public bool IsReadyForForceCheck()
		{
			if (!IsStarted || !IsRunning)
			{
				Debug.LogWarning(LogPrefix + "Detector should be started to use forced check.");
