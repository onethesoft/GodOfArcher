        ...
        public void Refresh(int defaultIndex = 0)
        {
            ...
            switch (tab.name)
            {
                ...
                case "Boots":
                {
                    ...
                    Item.GetParams = item => new ItemParams { Id = item.Id.Replace(".Armor.", $".{tab.name}."), Path = dict[item.Id] == null ? null : dict[item.Id].Path.Replace("Armor/", $"{tab.name}/") + $".{tab.name}", Meta = dict[item.Id] == null ? null : JsonConvert.SerializeObject(dict[item.Id].Tags) };
                    break;
                }
                case "Shield":
                {
